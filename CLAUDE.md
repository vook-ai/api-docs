...
- Reference currently covers **read endpoints only**; write endpoints render
  automatically once added to the public spec.
- Brand color: `#0883C6`. American English. No invented API facts.

## Writing style

Follow `.claude/rules/001-tone-style.md` for all docs copy. Applies to product
docs (not just emails, ignore the email-only bits like greetings/signatures):
...
the response?
- **No "no X, no Y" anaphora** (e.g. "No browser, no login flow."). This
  punchy negative-parallelism cadence reads as marketing copy. State it plainly
  instead ("You authenticate with an API key alone.").
