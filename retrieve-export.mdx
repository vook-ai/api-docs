...
| ----------------- | ------- | ------------------------------------------------------------- |
| `format`          | string  | Output format: `pdf`, `docx`, `srt`, `md`, `html`, or `json`. |
...
<Note>
  ...
  carries timestamps and omits speaker labels. The same applies to a `json`
  export, which returns the raw structured transcript.
</Note>
