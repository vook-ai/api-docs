...
`GET /api/v1/transcription-jobs`, which accepts the same `page` and `page_size`
parameters and returns the same page shape.
...
same content split into paragraphs, each with a list of timed `children` text
segments. `speaker_key` is present when you requested `diarize: true`.
