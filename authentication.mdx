...
Alongside the key, the app shows a **webhook signing secret**, also only once.
You use it to verify that webhook requests come from Vook (see
[Verify the signature](/webhooks#verify-the-signature)). Store it with the key.
It only verifies deliveries, so it cannot authenticate API requests.
...
- A revoked or expired key is rejected on the next request, so mint a new one to
  continue.
- Revoking a key stops new requests with it. Jobs it already submitted still
  run to completion.
- A signing secret belongs to its key and cannot be rotated on its own. If the
  secret leaks, mint a new key and revoke the old one.
