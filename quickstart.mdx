...
This guide walks through reading a transcription with the Vook API: list what you
have, check a transcription's status, fetch its text, and download an export.
Every request uses your API key as a Bearer token.

<Note>
  You create transcriptions in the Vook web app. This guide covers retrieving and
  exporting them through the API. The base URL is
...
```json
{
  ...
  "transcription": [ /* structured paragraphs */ ],
  "word_count": 1250
}
```
...
The `format` query parameter is required. Supported values: `pdf`, `docx`,
`srt`, `md`, `html`, `json`.

<CodeGroup>

```bash curl
curl "https://www.api.vook.ai/api/v1/transcriptions/$ID/export?format=pdf" \
  -H "Authorization: Bearer $VOOK_API_KEY" \
  -o transcript.pdf
```
