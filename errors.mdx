...
| `url_not_accessible` | `400` | The source `url` answered with a `4xx` (for example a typo, a private file, or an expired signed URL). Fix the URL. |
| `unauthorized` | `401`, `403` | The API key is missing, invalid, revoked, or expired. See [Authentication](/authentication). |
| `insufficient_api_credits` | `402` | Your API credit balance needs a top-up. Top up, then send the request again. |
...
| `insufficient_api_credits` | Your API credit balance needed a top-up before the job could start. |
