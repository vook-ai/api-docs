...
- **`4xx`**: the request will not succeed as sent. Change the request before
  you send it again. Two exceptions: `409 chat_turn_in_progress` succeeds once
  the current answer on that chat is ready, and `409 upload_not_found` succeeds
  once the file upload is stored.
